	...
	def gcfunc_set_axis_value(self, gcline: GCLine, **kwargs) -> list[GCLine]:
		...
		for gcline in Printer.gcfunc_set_axis_value(self, gcline) or [gcline]:
			if 'A' in gcline.args:
				...
				self.ring.angle += gcline.args['A'] if isinstance(gcline.args['A'], Angle) else Angle(degrees=gcline.args['A'])
				self.ring.angle %= 360
				...
				current_thread = GSegment(self.anchor, self.ring.point).as2d()
				self.gcode_ring_angle = self._calc_new_ring_angle(current_thread, self.y)
				gclines.append(gcline.copy(args={'A': self.gcode_ring_angle.degrees},
					add_comment=(f'Ring at {self.ring.angle:.3f}°, gcode angle at {self.gcode_ring_angle:.3f}°,',
					f'change requested was {gcline.args["A"]}')))
				if gcline.is_xymove():
	...
	def gcode_ring_move(self, dist, pause_after=False) -> list[GCLine]:
		...
		with Saver(self.head_loc, 'z') as saver:
			gcode += self.execute_gcode([
				...
				GCLine('G0', args={'A':dist.degrees % 360}, comment=f'({self.gcode_ring_angle+dist:.3f}°)')
			])
			if pause_after:
